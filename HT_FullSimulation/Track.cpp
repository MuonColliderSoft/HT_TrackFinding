...
void Track::print(std::ostream &out, int mode = 0){
    ...
    int i,j,k;
    int retcode = trackToCell(i, j, k, *this);
       
	out << "Track ID: " << ID << " mass: " << mass << " beta: " << beta << " vertex: x: "  
	<<  x0  << " y: "  <<  y0  << " z: "  <<  z0 << " t: " <<  t0
	<< " charge: " << charge << " invPt: "  << invPt  << " eta: " <<  eta 
	<< " phi: "  <<  phi << " [" << i << "," << j << "," << k << "]" <<  endl;

        
    if(mode == 0) return;
    
    out << hitList.size() << " hits" << endl;
...
void Track::write(std::ostream &out){// write track to a file
	...
	out << ID << " " << mass << " " << beta << " " << x0 << " " << y0 << " " << z0
		 << " " << t0 << " " << charge << " " << Pt << " " << eta << " " << phi << endl;
