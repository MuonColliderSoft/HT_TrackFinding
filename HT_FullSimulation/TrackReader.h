...
class TrackReader{
	...
	TrackReader(string fileName){ // constructor
	...
	bool read (Track &thisTrack){
	 ...
	 if(ientry != nEntries){
		...
		const float part_eta = asinh(1./tan(part_theta));
     	...
     	thisTrack.init(0., part_vx, part_vy, part_vz, part_t0*SpeedOfLight, part_q/part_pt, part_eta, part_phi); // specific track constructor
		...
		for (int ihit = 0; ihit != n_hit; ++ihit){
		  ...
		  const unsigned int sensor = (unsigned) ( (CellID >> 24) & 0xff );
		   ...
		   int layerInd = CellIDtoLayer(CellID);
			...
			Hit thisHit('H', CellID, layerInd, 0, 
        	x1, x2, t, x, y, z,
