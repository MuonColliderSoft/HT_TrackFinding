...
double xi_to_phi(double xi){return par.HTA_t_phi - par.HTA_t_deltaPhi + (2*par.HTA_t_deltaPhi/(double)par.HTA_NphiBins)*xi;}
double xj_to_eta(double xj){return par.HTA_t_eta - par.HTA_t_deltaEta + 2*(par.HTA_t_deltaEta/(double)par.HTA_NetaBins)*xj;}
double xk_to_invPt(double xk){return par.HTA_t_invPt_min + (par.HTA_t_invPt_max - par.HTA_t_invPt_min)/(double)par.HTA_NinvptBins*xk;}

double phi_to_xi(double phi){return (phi - par.HTA_t_phi + par.HTA_t_deltaPhi)/par.HTA_t_deltaPhi*(double)par.HTA_NphiBins/2.;}
double eta_to_xj(double eta){return (eta - par.HTA_t_eta + par.HTA_t_deltaEta)/par.HTA_t_deltaEta*(double)par.HTA_NetaBins/2.;}
double invPt_to_xk(double invPt){return (invPt - par.HTA_t_invPt_min)/(par.HTA_t_invPt_max - par.HTA_t_invPt_min)*(double)par.HTA_NinvptBins;}
...
//					t0 is time of track at origin
	...
	//cout << "pars " << pars.phi << " " << pars.theta << " " << pars.R << " " << pars.z0 << " " << pars.t0 << " " << pars.beta << endl;	
...
vector< vector <long int> > combinations; // all hit combinations to be fitted in one candidate
...
//const float K = 0.000299792458*MagneticField;
	...
	HelixPars hPars;
	 ...
	 vector<double>fitTimes;
	 unsigned nPoints = fitHitList.size();
	 for(int i = 0; i != nPoints; ++i) fitTimes.push_back(x[6+i]);
	...
	/*	
		cout << " " << (deltax1/errx1)*(deltax1/errx1);
		cout << " " << (deltax2/errx2)*(deltax2/errx2);	
		cout << " " << (deltax3/errx3)*(deltax3/errx3);
		cout << " " << (deltat/errt)*(deltat/errt);
		cout << endl;
	*/
	}
...
////////////////////////////////////////////////////////////////////////////////////////
    ...
    public:
        ...
        map<int, Hitstat>::iterator it, it1,it2;
        ...
        // The ID's point into hitList member of Event.
         ...
         map<int, vector<long int>> layerIndHitIDList;
...
////////////////////////////////////////////////////////////////////////////////////////
        ...
        void train(Hit &h){
        	...
        	if(TrainingPhase == 1)layerIndHitStat[h.CellID].train(h);
...
////////////////////////////////////////////////////////////////////////////////////////
        ...
        void print(ostream &out){
        	vector< map<int, Hitstat>::iterator> itList;
        	...
        	// Sort the vector in descending order using a lambda
   			std::sort(itList.begin(), itList.end(), [](map<int, Hitstat>::iterator it1, map<int, Hitstat>::iterator it2) {
        		return decodeLayer(it1->first) < decodeLayer(it2->first);
   			});
   			
   			cout << endl;
   			for(int i = 0; i != (int)itList.size(); ++i){
   				it = itList[i]; 				
        		out << "   sensor: " << it->first << " layer: " << decodeLayer(it->first) << " ";
...
////////////////////////////////////////////////////////////////////////////////////////
    ...
    //std::cerr << "DEBUG: fill() called. layerIndHitStat addr=" << &layerIndHitStat
			...
			if(it != layerIndHitStat.end()){
				...
				if((int)layerIndHitIDList[layerInd].size() == 1) ++nHitLayers;
...
////////////////////////////////////////////////////////////////////////////////////////
        ...
        static bool compareComb(vector<long int> v1, vector<long int> v2) {
...
////////////////////////////////////////////////////////////////////////////////////////
        ...
        if(true) cout << "fitCandidate " << i_index << " " << j_index << " " << k_index << endl;
			...
			vector< vector <long int> > combinations2; // all hit combinations to be fitted in one candidate
			vector< vector <long int> > *comb1 = &combinations;
			vector< vector <long int> > *comb2 = &combinations2;
			...
			vector< vector <long int> > finalCombinations;
			vector <long int> tempHitList;
			...
			for(int iComb = 0; iComb != nCombs; ++iComb){ 
				...
				for(int i = 0; i != (int)combinations[iComb].size(); ++i){
					...
					if(thisHitID != -1) tempHitList.push_back(thisHitID);
				}
									
				// check for minimum number of hits	in this combination
				
				
				unsigned minHits = max(nHitLayers - par.reco_maxDroppedLayers, par.reco_minLayersForFit) ;
								
				if(tempHitList.size() >= minHits) finalCombinations.push_back(tempHitList);		
			...
			for(int iComb = 0; iComb != nCombs; ++iComb){ 
				...
				for(int iFitPar = 6; iFitPar != totFitPars; ++iFitPar){
				
					min->SetVariable(iFitPar,("TOF"+to_string(iFitPar-6)).c_str(),fitHitList[iFitPar-6].t, 0.1);
...
////////////////////////////////////////////////////////////////////////////////////////
		...
		void initHists(){
			...
			H3D_HTAtrainStat = new TH3I("H3D_HTAtrainStat","H3D_HTAtrainStat; i; j; k",
							NphiBins,-0.5, NphiBins-0.5,
							NetaBins, -0.5,NetaBins-0.5,
							NinvptBins, -0.5, NinvptBins-0.5);
							...
							for(int j = 0; j != NetaBins; ++j)
								for(int k = 0; k != NinvptBins; ++k){
									...
									out << endl << "cell ijk: " << i << " " << j << " " << k << "      " 
										...
										<< " invPt: [" << invPtMin + k*invPtStep << "," << invPtMin + (k+1)*invPtStep << "]" << endl ;
...
////////////////////////////////////////////////////////////////////////////////////////
		...
		int read(istream &in){
			...
			for(; in ;){
				...
				if(i < 0 || i >= (int)NphiBins) return -1;
				if(j < 0 || j >= (int)NetaBins) return -1;
				if(k < 0 || k >= (int)NinvptBins) return -1;
				Hitstat hs;
				hs.mean[0] = mean[0];
				hs.mean[1] = mean[1];
				hs.mean[2] = mean[2];
...
////////////////////////////////////////////////////////////////////////////////////////
    ...
    void writeCellCandidateList(ostream &out){
...
////////////////////////////////////////////////////////////////////////////////////////
    ...
    string makeFileName(string prefix,int i, int j, int k){ 	
