 ...
 * @return best x find.
 */
template <int dim> Eigen::Matrix<double, dim, 1> Nelder_Mead_Optimizer(double (*func)(Eigen::Matrix<double, dim, 1>),double &best, Eigen::Matrix<double, dim, 1> x_start, double step=0.1,
                                                                       double no_improve_thr=10e-6, int no_improv_break=10, int max_iter=1000, double alpha=1.0, double gamma=2.0,
                                                                       double rho=-0.5, double sigma=0.5, bool log=false) {
    //double best, prev_best = func(x_start);
    double prev_best = func(x_start);
    int no_improv = 0;
    ...
    iteration++; //cout << "*** iteration " << iteration << endl;
